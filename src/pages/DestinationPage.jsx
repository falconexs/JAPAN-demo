...
export default function DestinationPage({ city }) {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (isCompact) {
        gsap.set(".destination-scene-copy > *", { opacity: 1, y: 0 });
      }
      ...
      sceneTl
        .fromTo(
          ".destination-bg img",
          { scale: isCompact ? 1 : data.bgScaleStart ?? 1.02, yPercent: 0 },
          { scale: isCompact ? 1.025 : data.bgScaleEnd ?? 1.08, yPercent: isCompact ? -1 : -2.5, ease: "none" },
          0
        )
        .fromTo(
          ".destination-building",
          { opacity: 0, y: isCompact ? 42 : 92, clipPath: isCompact ? "inset(8% 0 0 0)" : "inset(16% 0 0 0)" },
      ...
      <section className="destination-scroll-scene" aria-label={`${data.name} poster scene`}>
        <div className="destination-sticky">
          <div className="destination-bg" aria-hidden="true">
            <img src={data.bg} alt="" fetchPriority="high" decoding="async" />
          </div>
          <div className="destination-building-layer" aria-hidden="true">
            <div className="destination-building">
              <img src={data.building} alt="" fetchPriority="high" decoding="async" />
            </div>
          </div>
